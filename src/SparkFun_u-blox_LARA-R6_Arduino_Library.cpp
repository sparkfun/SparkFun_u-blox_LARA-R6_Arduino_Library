...
bool LARA_R6::processURCEvent(const char *event)
{
  ...
  { // URC: +UUPSDA (Packet Switched Data Action)
    ...
    char *searchPtr = strstr(event, LARA_R6_MESSAGE_PDP_ACTION_URC);
    if (searchPtr != nullptr)
    {
      searchPtr += strlen(LARA_R6_MESSAGE_PDP_ACTION_URC); // Move searchPtr to first character - probably a space
      while (*searchPtr == ' ') searchPtr++; // skip spaces
...
void LARA_R6::pruneBacklog()
{
  ...
  while (event != nullptr) //If event is actionable, add it to pruneBuffer.
  {
    ...
    if ((strstr(event, LARA_R6_READ_SOCKET_URC) != nullptr)
        ...
        || (strstr(event, LARA_R6_MESSAGE_PDP_ACTION_URC) != nullptr)
